          ...
          <div className="hidden md:flex items-center gap-4">
            {user ? (
              ...
              <div className="flex items-center gap-4">
                ...
                <Link 
                  href={user.role === 'driver' ? '/drive/dashboard' : user.role === 'admin' ? '/admin' : '/account'} 
              ...
              // Logged Out State
              <>
                <Link href="/auth" className="text-sm font-bold text-slate-900 hover:text-blue-600 transition">
                  Log In
                </Link>
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-40 bg-white pt-24 px-6 md:hidden animate-in slide-in-from-top-10 duration-200">
          <div className="flex flex-col space-y-2">
            ...
            {user && (
              <MobileLink 
                href={user.role === 'driver' ? '/drive/dashboard' : user.role === 'admin' ? '/admin' : '/'} 
            ...
            <div className="pt-6 mt-4 border-t border-slate-100">
              {user ? (
                ...
                <div className="space-y-4">
                  <Link href="/auth" onClick={() => setIsMenuOpen(false)} className="block w-full text-center text-slate-900 font-bold py-3 border border-slate-200 rounded-xl">
                    Log In
                  </Link>
