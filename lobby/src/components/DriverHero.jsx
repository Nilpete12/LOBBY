...
import { Power, ShieldCheck, MapPin, Radio } from "lucide-react";

export default function DriverHero({ userName = "Nilesh" }) {
  // Local state to toggle driver availability. 
  // You can connect this to your MongoDB API (/api/driver/update) later!
  ...
  const handleToggle = async () => {
    ...
    // Optional: Call your backend to update MongoDB availability in real-time
    /*
    try {
  ...
  return (
    <section className="relative pt-28 md:pt-36 pb-16 min-h-[75vh] flex items-center justify-center bg-[#F8FAFC] overflow-hidden transition-colors duration-500">
      ...
      <div className="max-w-7xl mx-auto px-5 md:px-6 relative z-10 w-full flex flex-col md:flex-row items-center justify-between gap-12">
        ...
        <div className="flex-1 w-full max-w-2xl text-center md:text-left">
          ...
          <div className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-white border border-slate-200/80 shadow-xs mb-6 transition-all">
            ...
            <span className="text-xs font-bold uppercase tracking-wider text-slate-700">
              {isOnline ? "Broadcasting Location • Active" : "Status • Currently Offline"}
            </span>
          </div>
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3.5 text-left">
                <div className={`p-3.5 rounded-2xl transition-colors duration-300 ${
                  isOnline ? "bg-teal-50 text-[#0F766E]" : "bg-slate-100 text-slate-400"
                }`}>
                  <Radio size={26} className={isOnline ? "animate-pulse" : ""} />
                </div>
                <div>
                  ...
                  <p className="text-xs font-semibold text-slate-400">
                    {isOnline ? "Direct Contact Active" : "No requests incoming"}
                  </p>
                </div>
              </div>
              ...
              <button
                ...
                className={`flex items-center gap-2.5 px-6 py-4 rounded-2xl font-bold text-sm transition-all duration-300 shadow-md active:scale-95 cursor-pointer ${
                  isOnline
                    ? "bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200/60 shadow-rose-500/10"
                    : "bg-[#0F766E] hover:bg-[#0d625b] text-white shadow-[#0F766E]/20 hover:shadow-lg hover:shadow-[#0F766E]/30"
                }`}
              >
                <Power size={18} />
        ...
        {/* RIGHT COLUMN: 3D VISUAL (Hidden on mobile to save space) */}
        <div className="hidden md:flex flex-1 justify-end items-center relative">
          ...
          <div className={`relative w-full max-w-115 h-90 transition-all duration-700 ease-out z-10 ${
            isOnline ? "hover:-translate-y-3 drop-shadow-[0_25px_35px_rgba(15,118,110,0.25)]" : "opacity-60 grayscale-40"
          }`}>
            <Image 
              src="/3d-taxi.png" 
